"""
Run a controlled ablation for attribute expert variants.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Attribute expert ablation")
...
def _run_variant(root: str, attribute_expert_variant: str, steps: int, seed: int) -> dict:
    run_experiment(
        train_mode="staged",
        n_steps=steps,
        results_root=root,
...
def main() -> None:
    ...
    root = args.results_root or os.path.join(
        ...
        "results",
        f"attribute_expert_ablation_{stamp}",
    )
    os.makedirs(root, exist_ok=True)

    variants = ["base", "normal_mixed", "wave_mixed", "mixed"]
    summary: dict[str, dict] = {}
    for variant in variants:
        summary[variant] = _run_variant(os.path.join(root, variant), variant, args.steps, args.seed)

    base = summary["base"]
    branch_name = base["stress_metrics"].get("directional_branch_name", "anisotropy_directional")
    ...
    report_lines.extend(
        [
            ...
            "",
            "| Variant | Branch stress MAE | Branch stress MaxErr |",
            "| --- | ---: | ---: |",
        ]
    )
    for variant in variants:
        branch = summary[variant]["stress_metrics"]["experts"][branch_name]
        report_lines.append(
            f"| `{variant}` | `{branch['mae']:.6f}` | `{branch['max_error']:.6f}` |"
        )

    with open(os.path.join(root, "ablation_summary.md"), "w", encoding="utf-8") as f:
        f.write("\n".join(report_lines) + "\n")

    print(f"[OK] Saved attribute expert ablation to: {root}")
